...
class _FoodMenuScreenState extends State<FoodMenuScreen> {
  ...
  Future<void> _loadItemsForCategory() async {
    ...
    try {
      ...
      if (currentCategoryID == '0') {
        rawItems = await isar.foodItemList
            .where()
            .filter()
            .isRecommendEqualTo('Y')
            .findAll();
      } else {
        rawItems = await isar.foodItemList
            .where()
            .filter()
            .food_category_IDEqualTo(currentCategoryID)
            .findAll();
      }
      ...
      for (var item in rawItems) {
        bool isServeMatch =
            (serveType == "ServeTable" && item.isServeTable == "Y");
        bool isTakeAwayMatch =
            (serveType == "TakeAway" && item.isTakeAway == "Y");
        ...
        var itemSizes = await isar.foodItemSizeList
            .where()
            .filter()
            .food_item_IDEqualTo(item.id)
            .findAll();
        for (var isz in itemSizes) {
          var sizeData = await isar.foodSizeList
              .where()
              .filter()
              .idEqualTo(isz.food_size_ID)
              .findFirst();
          if (sizeData != null) {
        ...
        var options = await isar.foodOptionList
            .where()
            .filter()
            .food_item_IDEqualTo(item.id)
            .findAll();
        ...
        for (var opt in options) {
          ...
          var choices = await isar.foodChoiceList
              .where()
              .filter()
              .parentIDEqualTo(opt.id)
              .findAll();
          choices.sort((a, b) => (a.seq ?? 0).compareTo(b.seq ?? 0));
          for (var c in choices) {
            var cvm = FoodChoiceViewModel(data: c);
            cvm.choiceSizes = await isar.foodChoiceSizeList
                .where()
                .filter()
                .food_choice_IDEqualTo(c.id)
                .findAll();
            ovm.choices.add(cvm);
          }
          vm.options.add(ovm);
        ...
        var sChoices = await isar.foodChoiceList
            .where()
            .filter()
            .parentIDEqualTo(item.id)
            .findAll();
        ...
        for (var choice in opt.choices) {
          if (choice.isSelected) {
            ...
            if (vm.selectedSizeID != null) {
              var sizeMatch = choice.choiceSizes
                  .where((cs) => cs.food_size_ID == vm.selectedSizeID)
                  .firstOrNull;
  ...
  void _handleSelectOptionChoice(
    String itemID,
    String optionID,
    String choiceID,
  ) {
    ...
    if (item != null) {
      ...
      if (opt != null) {
        ...
        var target = opt.choices
            .where((c) => c.data.id == choiceID)
            .firstOrNull;
        if (target != null) target.isSelected = true;
        _calculateTotal();
      }
  ...
  void _handleSelectStandaloneChoice(String itemID, String choiceID) {
    ...
    if (item != null) {
      ...
      var target = item.standaloneChoices
          .where((c) => c.data.id == choiceID)
          .firstOrNull;
      if (target != null) target.isSelected = true;
      _calculateTotal();
  ...
  Future<int> _getOrderNumber() async {
    ...
    final ordersToday = await isar.foodOrderList
        .where()
        .filter()
  ...
  Future<void> _orderFood() async {
    ...
    try {
      ...
      await isar.writeTxn(() async {
        ...
        if (receiptID != null) {
          existingReceipt = await isar.receiptList
              .where()
              .filter()
              .idEqualTo(receiptID)
              .findFirst();
        ...
        if (existingReceipt != null) {
          existingReceipt.sumAmount =
              (existingReceipt.sumAmount ?? 0.0) + totalAmount;
          existingReceipt.totalAmount =
              (existingReceipt.totalAmount ?? 0.0) + totalAmount;
        ...
        if (receiptID != null) {
          existingOrder = await isar.foodOrderList
              .where()
              .filter()
              .parentIDEqualTo(receiptID)
              .findFirst();
        } else if (widget.shop_open_table_ID != null) {
          existingOrder = await isar.foodOrderList
              .where()
              .filter()
              .parentIDEqualTo(widget.shop_open_table_ID)
              .findFirst();
        }
        ...
        if (existingOrder != null) {
          ...
          existingOrder.orderAmount =
              (existingOrder.orderAmount ?? 0.0) + totalAmount;
      ...
      if (mounted) {
        if (isDebug) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Debug: Order Generated')),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                isThai ? 'สั่งอาหารสำเร็จ' : 'Order Placed Successfully',
              ),
            ),
          );
          if (receiptID != null) {
            Navigator.pushReplacement(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    PaymentScreen(config: widget.config, receiptID: receiptID!),
              ),
            );
          }
        }
    ...
    return Scaffold(
                ...
                child: Row(
                  children: [
                    _buildServeTypeButton(
                      'ServeTable',
                      isThai ? 'ทานที่ร้าน' : 'Serve Table',
                    ),
                    _buildServeTypeButton(
                      'TakeAway',
                      isThai ? 'สั่งกลับบ้าน' : 'Take Away',
                    ),
                  ],
                ),
              ),
      ...
      body: isLoading
          ...
          : ListView.builder(
              ...
              itemCount:
                  itemList.length + (isDebug && _debugText.isNotEmpty ? 1 : 0),
              itemBuilder: (context, index) {
                ...
                bool hasPicture =
                    item.data.localPicture != null &&
                    item.data.localPicture!.isNotEmpty;
                File? imageFile = hasPicture
                    ? File(item.data.localPicture!)
                    : null;
                          ...
                          Wrap(
                            ...
                            children: [
                              ...item.sizes.map(
                                (s) => ChoiceChip(
                                  label: Text(
                                    '${isThai ? s.data.thaiName : s.data.englishName} ${s.itemSizeData.price?.toStringAsFixed(0) ?? ''}',
                                  ),
                                  selected: s.isSelected,
                                  showCheckmark: false,
                                  onSelected: (_) => _handleSelectSize(
                                    item.data.id!,
                                    s.data.id!,
                                  ),
                          ...
                          Wrap(
                            ...
                            children: [
                              ...opt.choices.map(
                                (c) => ChoiceChip(
                                  label: Text(
                                    '${isThai ? c.data.thaiName : c.data.englishName}${_getDisplayChoicePrice(c)}',
                                  ),
                                  selected: c.isSelected,
                                  showCheckmark: false,
                                  onSelected: (_) => _handleSelectOptionChoice(
                                    item.data.id!,
                                    opt.data.id!,
                                    c.data.id!,
                                  ),
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: item.quantity > 0
                                        ? () =>
                                              _handleQuantity(item.data.id!, -1)
                                        : null,
                                  ),
                                  IconButton(
                                    icon: const Icon(
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
            isThai
                ? 'คุณต้องการยกเลิกการสั่งอาหารใช่หรือไม่?'
