...
class MerchandiseStockScreen extends StatefulWidget {
  final EnvConfig config;
  const MerchandiseStockScreen({super.key, required this.config});

  @override
  State<MerchandiseStockScreen> createState() => _MerchandiseStockScreenState();
}

class _MerchandiseStockScreenState extends State<MerchandiseStockScreen> {
            ...
            PopupMenuButton<int>(
              ...
              onSelected: (value) {
                if (value == 1) {
                  ...
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => DocumentTypeScreen(config: widget.config)));
                }
              },
              itemBuilder: (context) => [
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _buildTopHeader(),
          Expanded(
            child: Center(child: Text(isThai ? 'สต็อก' : 'Stock')),
          ),
        ],
      ),
    );
