...
class _FoodMenuScreenState extends State<FoodMenuScreen> {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(isThai ? 'คุณต้องการยกเลิกการสั่งอาหารใช่หรือไม่?' : 'Do you want to cancel this order?'),
          actions: [
            TextButton(
  ...
  Future<void> _processCancel() async {
    if (widget.receiptID != null) {
      final receipt = await isar.receiptList.where().filter().idEqualTo(widget.receiptID!).findFirst();
      if (receipt != null) {
        await isar.writeTxn(() async {
