...
class _ReceiptScreenState extends State<ReceiptScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      var query = currentStatus == 'Wait4Payment'
          ? isar.receiptList
              .where()
              .filter()
              .createdAtGreaterThan(DateTime.parse(todayStr))
              .and()
              .statusEqualTo('Wait4Payment')
              .or()
              .createdAtGreaterThan(DateTime.parse(todayStr))
              .and()
              .statusEqualTo('InformPayment')
          : isar.receiptList
              .where()
              .filter()
              .statusEqualTo(currentStatus == 'Served' ? 'Paid' : currentStatus)
              .and()
              .createdAtGreaterThan(DateTime.parse(todayStr));

      if (currentStatus == 'Paid' || currentStatus == 'Served') {
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: Row(
              children: [
                _buildStatusButton('Wait4Payment', isThai ? 'รอ' : 'Wait'),
                const SizedBox(width: 8),
                _buildStatusButton('Paid', isThai ? 'จ่าย' : 'Paid'),
                const SizedBox(width: 8),
                _buildStatusButton('Served', isThai ? 'เสิร์ฟ' : 'Served'),
                const SizedBox(width: 8),
                Expanded(
                  child: ElevatedButton(
                    style: ElevatedButton.styleFrom(
                      backgroundColor: currentStatus == 'Cancel'
                          ? Colors.blue
                          : Colors.grey[200],
                      foregroundColor: currentStatus == 'Cancel'
                          ? Colors.white
                          : Colors.black87,
                    ),
                    onPressed: () {
